def main(data):
    ...
    if '!tfw' in data['recv']:
        import random, urllib2, json, urllib
        args = argv('!tfw', data['recv'])
        query = ','.join(args['argv'][1:])
        url = "http://api.openweathermap.org/data/2.5/weather?"+ urllib.urlencode({'q': query, 'units': 'metric'})
        ...
        response = u"{temp} {remark} - {flavor} - {location} {time}Z".format(
